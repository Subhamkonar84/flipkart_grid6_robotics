...
def fetch_exp_by_name(product_name):

    if product_name in fresh or product_name[4:] in fresh:
        try:
        # Connect to the MySQL database
            conn = mysql.connector.connect(
            host='localhost',          # Replace with your host
            user='proj',               # Replace with your MySQL username
            password='project@123',    # Replace with your MySQL password
            database='flipkart'        # Replace with your database name
        )
            cursor = conn.cursor()

        # Query to fetch the expiry date based on the product name
            query = """
            SELECT expected_life_span
            FROM fresh
            WHERE product = %s
            """
        ...
        # If a row is returned, extract the expiry_date
            if row:
                ...
                # Add the number of days from row[0]
                new_expiry_date = current_datetime + timedelta(days=row[0])
        ...
        try:
            ...
            # If a row is returned, extract the expiry_date
            if row:
                expiry_date = row[0]  # Extract the first column value
                return expiry_date
            else:
                print(f"No matching product found for {product_name}")
                return None  # Explicitly return None if no record is found
...
def run_check():
    while True:
        check_and_update_dict(result_dict)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    key = f"{names[int(c)]}{'s' * (n > 1)}"  # Form the key
                    value = n  # Form the value
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            ...
            detectt=threading.Thread(target=detect)
            runn= threading.Thread(target=run_check)
